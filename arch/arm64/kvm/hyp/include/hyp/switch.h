...
#define compute_guest_fgt_reg_clr_set(vcpu, reg, clr, set)		\
	do {								\
...
#define compute_fgt_clr_set(vcpu, reg, clr, set)			\
	do {								\
		u64 hfg = vcpu->arch.fgt[reg ## _REG];			\
		struct fgt_masks *m = reg_to_fgt_masks(reg);		\
		set |= hfg & m->mask;					\
		clr |= hfg & m->nmask;					\
	} while(0)
...
#define update_fgt_traps_cs(hctxt, vcpu, reg, clr, set)			\
	do {								\
		...
		if (is_nested_ctxt(vcpu))				\
			compute_guest_fgt_reg_clr_set(vcpu, reg, c, s);	\
									\
		compute_fgt_clr_set(vcpu, reg, c, s);			\
									\
		val = m->nmask;						\
		val |= s;						\
		val &= ~c;						\
...
static inline void __activate_traps_hfgxtr(struct kvm_vcpu *vcpu)
{
	...
	if (cpu_has_amu())
		update_fgt_traps(hctxt, vcpu, HAFGRTR_EL2);

	if (!cpus_have_final_cap(ARM64_HAS_FGT2))
	    return;

	update_fgt_traps(hctxt, vcpu, HFGRTR2_EL2);
	update_fgt_traps(hctxt, vcpu, HFGWTR2_EL2);
	update_fgt_traps(hctxt, vcpu, HFGITR2_EL2);
	update_fgt_traps(hctxt, vcpu, HDFGRTR2_EL2);
	update_fgt_traps(hctxt, vcpu, HDFGWTR2_EL2);
}
